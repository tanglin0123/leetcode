package leetcode.problem321.method2;

import java.util.*;

//88 / 102 test cases passed.
...
public class Solution {
    public int[] maxNumber(int[] nums1, int[] nums2, int k) {
        ...
        while(true){
            List<int[]> newCan = new LinkedList<int[]>();
            int newCanMax = -1;
            
            while(!candidates.isEmpty()){
                int[] can = candidates.remove(0);
            
                int len1 = nums1.length - can[0];
                int len2 = nums2.length - can[1];
        
                if(remain > len1 + len2){
                    continue;
                }
        
                int searchDist = len1 + len2 - remain;
        
                int max1 = can[0] < nums1.length ? nums1[can[0]] : -1;
                int maxIdx1 = can[0] < nums1.length ? can[0] : -1;
                for(int idx = can[0], i = 0; idx < nums1.length && i <= searchDist; ++idx, ++i){
                    if(max1 < nums1[idx]){
                        max1 = nums1[idx];
                        maxIdx1 = idx;
                    }
                ...
                int max2 = can[1] < nums2.length ? nums2[can[1]] : -1;
                int maxIdx2 = can[1] < nums2.length ? can[1] : -1;
                for(int idx = can[1], i = 0; idx < nums2.length && i <= searchDist; ++idx, ++i){
                    if(max2 < nums2[idx]){
                        max2 = nums2[idx];
                        maxIdx2 = idx;
                    }
                ...
                if(max1 > max2){
                    if(max1 >= newCanMax) {
                        ...
                        r[k - remain] = max1;
                        newCan.add(new int[]{maxIdx1 + 1, can[1]});
                    }
                } else if(max1 < max2){
                    if(max2 >= newCanMax) {
                        if(max2 > newCanMax){
                        ...
                        r[k - remain] = max2;
                        newCan.add(new int[]{can[0], maxIdx2 + 1});
                    }
                } else { // max1 == max2
                    if(max1 >= newCanMax) {
                        if(max1 > newCanMax){
