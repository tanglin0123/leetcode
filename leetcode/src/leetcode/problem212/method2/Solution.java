package leetcode.problem212.method2;
...
public class Solution {
    ...
    private boolean exist(char[][] board, String word, boolean[][] used, int index, int i, int j){
        int h = board.length;
        int w = board[0].length;
        ...
        boolean found = false;
        if(i-1 >=0 && !used[i-1][j]){
            found = exist(board, word, used, index+1, i-1, j);
            if(found){
                used[i][j] = false;
                return true;
            }
        ...
        if(j-1 >=0 && !used[i][j-1]){
            found = exist(board, word, used, index+1, i, j-1);
            if(found){
                used[i][j] = false;
                return true;
            }
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i < chs.length; ++i){
			chs[i] = board[0].toCharArray(); 
		}
		
		Solution s = new Solution();
		List<String> r = s.findWords(chs, words);
		for(String str : r){
